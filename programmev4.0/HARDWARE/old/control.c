...
/*	if(ALL_flag.unfold_rd)
	...
	{		
	  ...
	  case WAITING_1: //等待解锁
			...
			Angle.yaw =   pidYaw.measured = 0;   //锁定偏航角
		 
		  pidYaw.desired =90;//设定目标值
      ...
      pidRateX.measured = MPU6050.gyroX * Gyro_G; //内环测量值 角度/秒
		 ...
		 pidRoll.measured = Angle.roll;
			...
			pidUpdate(&pidYaw,dt);    //调用PID处理函数来处理外环	横滚角PID		
			pidRateX.desired = pidYaw.out; //将外环的PID输出作为内环PID的期望值即为串级PID
...
{	
	...
	if(ALL_flag.unlock == EMERGENT) //意外情况，请使用遥控紧急上锁，飞控就可以在任何情况下紧急中止飞行，锁定飞行器，退出PID控制
		...
		if(ALL_flag.unfold_rd==1&&ALL_flag.unlock==0)//已展开未解锁
	{ 
	...
	{		
		...
		case WAITING_2: //解锁完成后判断使用者是否开始拨动遥杆进行飞行控制
			...
			if(ALL_flag.flight_mode&&ALL_flag.maintain_hight&&Remote.thr<1200)
			{
				MOTOR1 = MOTOR2 = MOTOR3 = MOTOR4 = 200;
				status = WAITING_2;
			}
